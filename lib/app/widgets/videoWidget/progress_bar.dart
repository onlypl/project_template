...
class _ProgressBarState extends State<ProgressBar> {
  ...
  void _seekToRelativePosition(Offset globalPosition) {
    print(
      '设置播放位置--${context.calcRelativePosition(controller.value.duration, globalPosition)}',
    );
    controller.seekTo(
    ...
    return widget.draggableProgressBar
        ? GestureDetector(
          ...
          onHorizontalDragUpdate: (DragUpdateDetails details) {
            ...
            listener();
            Log().debug(
              '拖拽播放位置--${context.calcRelativePosition(controller.value.duration, _latestDraggableOffset!)}',
            );
            widget.onDragUpdate?.call(
              '${context.calcRelativePosition(controller.value.duration, _latestDraggableOffset!)}',
            );
          },
...
extension RelativePositionExtensions on BuildContext {
  Duration calcRelativePosition(Duration videoDuration, Offset globalPosition) {
    ...
    final double relative = (tapPos.dx / box.size.width).clamp(0, 1); //最大和最小值
    final Duration position = videoDuration * relative;
    // String durationStr = position.inSeconds.toString();
    // String newDurationStr = durationStr.split('.').first;
    // return Duration(seconds: int.parse(newDurationStr));
    return position;
  }
}
